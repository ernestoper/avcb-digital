...
const DetalheProcessoAdmin = () => {
  ...
  const handleApproveDocument = async () => {
    ...
    try {
      ...
      // Atualiza status da versão correspondente (traceability)
      try {
        if (hasVersionsTable) {
          await supabase
            .from("process_document_versions")
            ...
            .eq("file_url", selectedDocument.file_url);
        }
      } catch (verErr: any) {
        const msg = verErr?.message || "";
        if (msg.includes("Not Found") || msg.includes("404") || msg.includes("does not exist")) {
          setHasVersionsTable(false);
          console.warn("Tabela process_document_versions ausente; ignorando atualização de versão.");
        } else {
          console.error("Falha ao atualizar versão do documento:", verErr);
        }
      }
  ...
  const handleRejectDocument = async () => {
    ...
    try {
      ...
      // Atualiza status da versão correspondente
      try {
        if (hasVersionsTable) {
          await supabase
            .from("process_document_versions")
  ...
  const handleStamped = async ({ blob, filename, code }: { blob: Blob; filename: string; code: string }) => {
    ...
    try {
      ...
      // Registra versão carimbada, se a tabela existir
      try {
        if (hasVersionsTable) {
          const nextVersion = (versionsByDoc[stampDoc.id]?.length || 0) + 1;
          const { error: verError } = await supabase
            ...
            .insert({
              ...
              status: "completed",
              rejection_reason: null,
              correction_justification: "Documento carimbado e aprovado pelo CBM-PE",
            });
        ...
        if (msg.includes("Not Found") || msg.includes("404") || msg.includes("does not exist")) {
          setHasVersionsTable(false);
          console.warn("Tabela process_document_versions ausente; seguindo sem registrar versão.");
        } else {
          console.error("Falha ao registrar versão carimbada:", verErr);
        }
      }

      const { error: procError } = await supabase
        .from("processes")
        .update({ current_status: "concluido" })
        .eq("id", process.id);
      ...
      toast({ title: "Documento carimbado", description: "Versão carimbada gerada e processo concluído." });
      setStampOpen(false);
      setStampDoc(null);
      setStampUrl("");
      await fetchDocuments();
  ...
  const addToHistory = async (stepStatus: StepStatus, obs: string) => {
    ...
    await supabase.from("process_history").insert([{
      ...
      responsible_name: profile?.full_name || "Administrador",
    }]);
  };
      ...
      <Dialog open={actionDialog === "approve"} onOpenChange={(open) => {
        ...
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleApproveDocument} disabled={processing}>
